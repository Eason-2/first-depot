...
def _export_ai_toolbox_assets(project_root: Path, output_dir: Path, base_path: str) -> str:
    toolbox_dir = output_dir / 'ai-toolbox'
    toolbox_dir.mkdir(parents=True, exist_ok=True)
    assets_dir = project_root / 'apps' / 'api' / 'toolbox_assets'
    css_path = assets_dir / 'app.css'
    js_path = assets_dir / 'app.js'
    if css_path.exists():
        (toolbox_dir / 'app.css').write_text(css_path.read_text(encoding='utf-8'), encoding='utf-8')
    if js_path.exists():
        (toolbox_dir / 'app.js').write_text(js_path.read_text(encoding='utf-8'), encoding='utf-8')

    html = _apply_base_path(render_ai_toolbox_page(), base_path)
    css_ref = f"{base_path}/ai-toolbox/app.css" if base_path else '/ai-toolbox/app.css'
    js_ref = f"{base_path}/ai-toolbox/app.js" if base_path else '/ai-toolbox/app.js'
    html = html.replace('/api/ai-toolbox/assets/app.css', css_ref)
    html = html.replace('/api/ai-toolbox/assets/app.js', js_ref)
    return html
...
def main() -> None:
    ...
    parser.add_argument('--publish-dir', default='deliverables/published', help='Directory containing markdown posts.')
    parser.add_argument('--cname', default='', help='Optional custom domain (example: blog.example.com).')
    parser.add_argument('--base-path', default='', help='Optional URL base path for project pages (example: /first-depot).')
    args = parser.parse_args()

    project_root = Path(__file__).resolve().parents[1]
    output_dir = project_root / args.output_dir
    publish_dir = project_root / args.publish_dir
    result = export_static_site(project_root=project_root, output_dir=output_dir, publish_dir=publish_dir, cname=args.cname, base_path=args.base_path)
