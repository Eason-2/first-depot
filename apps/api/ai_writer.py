...
def render_ai_writer_page() -> str:
    ...
    return (
        ...
        ".back-link-button{display:inline-flex;align-items:center;justify-content:center;padding:8px 14px;border-radius:8px;border:1px solid #16a34a;background:#16a34a;color:#fff;text-decoration:none;font-weight:600;}"
        ".back-link-button:hover{background:#15803d;opacity:0.92;text-decoration:none;}"
        ...
        ".runtime-toggle{margin-top:0;margin-right:0;padding:12px 20px;font-size:24px;line-height:1;min-width:88px;}"
        ...
        "function errMsg(e){return e && e.name==='AbortError' ? '请求超时，请重试。' : (e && e.message ? e.message : STATIC_HINT);}"
        "async function parseJsonResponse(response){ const contentType = response.headers.get('content-type') || ''; const text = await response.text(); if(!contentType.includes('application/json')){ throw new Error(STATIC_HINT); } try{ return JSON.parse(text); } catch(e){ throw new Error(STATIC_HINT); } }"
        ...
        "async function loadRuntime(){ try{ const r = await fetch('/api/ai-writer/runtime'); const payload = await parseJsonResponse(r); if(!payload.ok){ throw new Error(payload.error || '加载失败'); } fillRuntimeForm(payload.data); renderRuntimeStatus(payload.data); show('runtime_apply', '当前挡位已加载。'); } catch(e){ show('runtime_apply', errMsg(e)); document.getElementById('runtime').textContent='当前为静态演示页'; } }"
